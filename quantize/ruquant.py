...
def ruquant(
    ...
    for i in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            # create optimizer
            optimizer = torch.optim.AdamW(
                [{"params":let_parameters(qlayer, use_shift),"lr":args.let_lr}, {"params":lwc_parameters(qlayer),"lr":args.lwc_lr},
                {"params":lh_parameters(qlayer),"lr":args.lh_lr}],weight_decay=args.wd)
            # {"params":let_parameters(qlayer, use_shift),"lr":args.let_lr}, {"params":lwc_parameters(qlayer),"lr":args.lwc_lr},
            loss_scaler = utils.NativeScalerWithGradNormCount()
            from tqdm import tqdm

            for epoch in tqdm(range(args.epochs), desc="Training Epochs", unit="epoch"):
            # for epochs in range(args.epochs):
                ...
                for j in range(args.nsamples//args.batch_size):  
                # for j in tqdm(range(args.nsamples//args.batch_size), desc="Training batch", unit="batch"):
                    index = j * args.batch_size
                    # obtain output of quantization model
                    with traincast():
                        post_rotate_quant_temporary(qlayer, args)
                        post_opt_omg_temporary(qlayer, args)
                        quant_out = qlayer(quant_inps[index:index+args.batch_size,], attention_mask=attention_mask_batch,position_ids=position_ids)[0]
                        loss = loss_func(fp_inps[index:index+args.batch_size,], quant_out)
                        if args.aug_loss:
                            loss += loss_func(fp_inps_2[index:index+args.batch_size,], quant_out)
