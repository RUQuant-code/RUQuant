...
class UniformAffineQuantizer(nn.Module):
    def __init__(
        ...
        self.random_permutation_times = random_permutation_times
        ...
        if self.rotate is None:
            ...
            if self.rotate is not False:
                self.init_ruquant_params = torch.tensor(0)
    ...
    def construct_givens_matrix_optimized(self,u, v):
        ...
        if not torch.allclose(norm_u_i, norm_v_i, atol=1e-6):
            raise ValueError("block norm mismatch")

        dot_product = torch.sum(u_chunks * v_chunks, dim=1)
        cross_product = v_chunks[:, 0] * u_chunks[:, 1] - v_chunks[:, 1] * u_chunks[:, 0]  
        ...
        return G
    def get_R(self,X):
        R=torch.eye(self.block_size,self.block_size,device=X.device)
        for i in range(self.max_rotation_step):
            ...
            omg_norm_sq = torch.dot(omg, omg)
            
            
            if omg_norm_sq < 1e-10:  
                print('omg is zero vector, skip this iteration')
                continue

            # H=torch.eye(omg.shape[0],device=X.device)-2/(omg.view(1,-1)@omg.view(-1,1))*(omg.view(-1,1)@omg.view(1,-1))
            H = torch.eye(omg.shape[0], device=X.device) - 2 * torch.outer(omg, omg) / omg_norm_sq
            X=H@X
            
            R=H@R
            for i in range(self.random_permutation_times):
    ...
    def online_ruquant_cali(self, weight):
        weight = weight.detach().clone()
        T = {}
    ...
    def init_ruquant(self, x: torch.Tensor):
    ...
    def register_ruquant_params(self):
        ...
        delattr(self, 'init_ruquant_params')
        self.register_buffer('omg', omg)
        self.register_buffer('permutation_list', permutation_list)
        self.register_buffer('R', R)
        self.register_buffer('init_ruquant_params', torch.tensor(1))

    def copy_ruquant_params(self, quantizer_ref):
        if quantizer_ref.rotate is True:
            assert quantizer_ref.init_ruquant_params == True
